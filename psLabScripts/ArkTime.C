...
class I3Ark : public Ark {
    public:
    EventLoader evLoader; //creates an object from class eventLoader in psLab/trunk/llh/public/EventLoader.h
    vector<I3Event> baseEvents; //creates a vector containing events of class I3Event, class I3Event in psLab/trunk/llh/public/I3Event.h
    //cout << 'baseEvents.GetMJD() ' << baseEvents.GetMJD() << endl;
    ...
    void SetPointSource(EquatorialDeg srcCoord, const FluxBase& flux, TimePdf * tPdf) {
        ...
        vector<I3Event> sourceEvents;
        evLoader.LoadSourceEvents(sourceEvents, mySrcLocation); //here select events that are within 1 deg (zenith) of point source
        TimePdf * tPdf1 = tPdf->Clone();
        tPdf1->CheckTimeBounds(tmin,tmax);
        I3PointGenerator i3point(sourceEvents, flux, mySrcLocation, livetime, tPdf1); // in llh/public/MultiAnalysisSet.h, what does it do exactly?
        (dynamic_cast<I3Analysis*> (psData))->SetSource(i3point);
        //(dynamic_cast<I3Analysis*> (psData))->GenerateDataSet_with_nSrcEvents(10)
    }
};