...
def main(argv):
    ...
    for i in range(1,h.GetNbinsX()+1):
        if len(medians[i-1])>3:
            ...
            go.SetPoint(npoints,h.GetXaxis().GetBinCenter(i),ROOT.RescaledSigma_IC40(1.,pow(10,h.GetXaxis().GetBinCenter(i))))
            npoints+=1
    go.Set(npoints)
    ...
    for i in range(0,tr.GetEntries()):
        ...
        pull=dAngle/ROOT.RescaledSigma_IC40(np.radians(sigma.value),ene.value)
