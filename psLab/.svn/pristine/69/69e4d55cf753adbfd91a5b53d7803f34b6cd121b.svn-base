...
// -----------------Reading Files from Script -------------------------------------
    ...
    ifstream cloud_data_count("SNR_cloud.txt"); // counting number of sources in table 
    int numlines = 0;
    while (getline(cloud_data_count, allLines)){
      ++numlines;
    }
    ...
    ifstream cloud_data("SNR_cloud.txt");
    for (int i=0; i<numlines; i++){
          cloud_data >> name[i];
          for (int j =0; j<4; j++){
             cloud_data >> properties[i][j];
          }
    }

    for(int i=0; i<numlines; i++){ // assigning values to table in script. 
       //cout<<properties[i][0]<<endl;
       //cout<<float(properties[i][0])<<endl;
