...
if __name__ == "__main__":
    ...
    for blockfilepath in files:
        ...
        if host == "submitter.icecube.wisc.edu":
            command = "\'/data/user/achristov/psLab/macro_llh/IC86-IV_TDep/qScripts_MESE/"+outfile+"'"
            print command
            call(["/home/achristov/bin/submit_npx4_long.sh", env, "root", "-l", "-b", "-q", command])
            
        else:
            print "This is not the cluster!"
            command = "/home/christov/psLab/macro_llh/IC86-IV_TDep/qScripts_MESE/"+outfile
            print command
