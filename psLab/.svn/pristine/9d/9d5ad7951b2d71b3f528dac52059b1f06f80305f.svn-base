...
def main(argv):
    ...
    for i in range(1,h.GetNbinsX()+1):
        if len(medians[i-1])>3:
            wp=weighted_percentile(0.5,np.asarray(medians[i-1]),np.asarray(mediansW[i-1]))
            if wp>30.: continue
            print wp 
    ...
    for i in range(1,h.GetNbinsX()+1):
        if len(medians[i-1])>3:
            ...
            #else:
                #plotZen=100.
            go.SetPoint(npoints,h.GetXaxis().GetBinCenter(i),ROOT.mpfSigmaDegRescaledIC59(1.,pow(10,h.GetXaxis().GetBinCenter(i))))
    ...
    for i in range(1,h2.GetNbinsX()+1):
        if len(medians2[i-1])>3:
            wp=weighted_percentile(0.5,np.asarray(medians2[i-1]),np.asarray(medians2W[i-1]))
            #if wp>30.: continue
    ...
    h3=TH2D("pull - corrected xcheck","pull - corrected xcheck",50,1,9,100,0.01,30)
    ...
    for i in range(0,tr.GetEntries()):
        ...
        w=weight(oneW.value,eneMC.value,nEv.value)
        dAngle=SpaceAngleRad(MCzenith.value,MCazimuth.value , np.radians(SMPEzenith.value), np.radians(SMPEazimuth.value))
        pull=dAngle/ROOT.mpfSigmaDegRescaledIC59(np.radians(sigma.value),ene.value)
        s_cor=ROOT.mpfSigmaDegRescaledIC59(np.radians(sigma.value),ene.value)
    ...
    for i in range(1,h2.GetNbinsX()+1):
        if len(medians3[i-1])>3:
            wp=weighted_percentile(0.5,np.asarray(medians3[i-1]),np.asarray(medians3W[i-1]))
            #if wp>30.: continue
