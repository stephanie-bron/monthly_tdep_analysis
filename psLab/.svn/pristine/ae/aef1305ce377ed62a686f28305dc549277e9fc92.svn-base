...
//  double GetPdfValue(Time t, double lag) { return GetPdfValue( t.GetMJD()-lag ); }
  ...
  void SetNorm() { norm_ = ( flareend_-flarebegin_)*flareheight_ + 
	                         ( tmax_-tmin_-(flareend_-flarebegin_))*quietheight_;}   
...
//  double GetPdfValue(Time t, double lag) { return GetPdfValue( t.GetMJD()-lag ); }
  ...
  void SetNorm() {
    double zlow  = TMath::Erf( (tmin_-mu_)/(sigma_*1.41421) );
    double zhigh = TMath::Erf( (tmax_-mu_)/(sigma_*1.41421) );
    norm_  = (zhigh-zlow)/2.;
  }
