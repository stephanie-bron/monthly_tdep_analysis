...
// -----------------Reading Files from Script -------------------------------------
    ...
    ifstream PWN_data_count("SNR_PWN.txt"); // counting number of sources in table 
    int numlines = 0;
    while (getline(PWN_data_count, allLines)){
      ++numlines;
    }
    ...
    ifstream PWN_data("SNR_PWN.txt");
    for (int i=0; i<numlines; i++){
          PWN_data >> name[i];
          for (int j =0; j<4; j++){
             PWN_data >> properties[i][j];
          }
    }
