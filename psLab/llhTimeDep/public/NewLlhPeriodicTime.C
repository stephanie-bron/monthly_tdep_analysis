...
double NewLlhPeriodicTimeFCN::operator() (const vector<double>& par) const
{
  ...
  for (int i=0;i<ptr->nEventsSelected_;i++)  { // loop over selected events
    ...
    TimeRatio = timePdf->GetPdfValue( ptr->tVect_[i] ) * ptr->livetime_ / b;
    ...
    if ( (ptr->monitorLevel_ > 3 && lh > 1.) || ptr->monitorLevel_ > 4 ) { 
      ...
      cout << lh                      << " " << 
              LogLambda               << " " << 
              ptr->spaceRatioVect_[i] << " " << 
              eRatio                  << " " << 
              ptr->tVect_[i]          << " " << 
              fmod(event->GetTime().GetMJD()-ptr->t0_,ptr->period_)/ptr->period_ << " " << 
              timePdf->GetPdfValue(ptr->tVect_[i]) << endl;
    }
  ...
  double nn = ptr->margPower_; 
  double mn = 1.;
  if (ptr->JimsTerm_ && sigma>0.1) { mn = TMath::Erf(0.5/sigma); }
  ...
  if ( ptr->JimsTerm_ && log(pow( mn * sigma * 2.0/ ptr->livetime_, nn )) < 0. ) { 
    
    LogLambda += log( pow( mn * sigma * 2.0/ ptr->livetime_, nn ) );
  }
 

  ptr->margValue_ = log( pow( mn * sigma * 2.0/ ptr->livetime_, nn ) );
  ...
  if ( ptr->monitorLevel_>1 ) { // more useful monitoring, with only
    ...
    printf("LogLambda=%12.6lg : nSrc=%9.4lg : gamma=%5.4lg : mean=%8.8lg : sima=%8.4lg : nEvs=%i : marg_fact=%g : mn=%f\n",
	   LogLambda, srcFrac*ptr->nEventsTot_, gamma, mean, sigma, ptr->nevs_, ptr->margValue_, mn );
  }  
...
NewLlhPeriodicTime::NewLlhPeriodicTime() :
  ...
  seedWtMin(1000)
{
  fcn_ = new NewLlhPeriodicTimeFCN();
  fcn_->Point(this);
...
void NewLlhPeriodicTime::MaximizeLlh()
{
  ...
  if (optParAuto_[0]) {
    ...
    minuit_->SetParameter(0, "nSrc", nsrcGuess_, 1., nSrcMin_, nSrcMax);
  }
  ...
  if (optParAuto_[1]) {
    if (optUseEnergy_) {
      ...
      minuit_->SetParameter(1, "gamma", gammaInit, 0.2, gammaMin_, gammaMax_);
    } else {
