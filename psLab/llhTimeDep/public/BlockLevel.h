...
class BlockTimePdf : public TimePdf {
 ...
 public:
  ...
  double GetPdfValue(double MJD){ 
    if (MJD<tmin_ ||MJD>tmax_){
        cout << "Icecube event is outside the time range of the lightcurve." << endl;
        return 0;
    }

    for (unsigned int i=0; i <= levels_.size(); i++) {
      if (levels_[i].BBegin <= MJD && (levels_[i].BBegin + levels_[i].BDuration) > MJD) { //= if icecube event is included in the block
        return levels_[i].BLevel/norm_; //divide this block level by the total space filled by blocks with certain level and duration, gives the relative 'weight' of this block
      }
    }
    return 0;
  }
...
//  double GetPdfValue(Time t, double lag) { return GetPdfValue( t.GetMJD()-lag ); }
  ...
  void fillHisto(int nbins){
    ...
    int nbins = levels_.size();
    double begin[nbins];
    for (int i=0; i<nbins; i++){
      begin[i] = levels_[i].BBegin;
    }
    histo_ = new TH1D("histo_","source",nbins,begin);
    double MJD;
...
class BlockTimePdf1 : public TimePdf {
 ...
 public:
  ...
  double GetPdfValue(double MJD){ //MJD = MJD-lag: virtually change the arrival time of the event so we have access to events arriving later than the flare
    ...
    if ( norm_<1e-20 ) {
        cout << "PDF set to 0 because no block is above the threshold for the arrival time of this event. Norm = " << norm_ << endl;
        return 0.; //saying no block is above the threshold
    }     

    //levels[i].BLevel = red line on graph
    for (unsigned int i=0; i <= levels_.size(); i++) {
...
//  double GetPdfValue(Time t, double lag) { return GetPdfValue( t.GetMJD()-lag ); }
  ...
  void fillHisto(int nbins=0){
    ...
    double tmax_ = tmin + livetimeTotal;
    double bintime = (tmax_ - tmin) / nbins;
    double MJD;
    histo = new TH1D("histo","source",nbins,tmin,tmax_);
    for (int i=1; i <= nbins; i++){ // bins go from 1 to n
      MJD = tmin + (i-0.5)*bintime;
      histo->SetBinContent(i,GetPdfValue(MJD));
    }
    //*/
  }
