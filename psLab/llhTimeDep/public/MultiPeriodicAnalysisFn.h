...
//    cout<<"Check weights : "<< nsrcMax<<" "<< nsrcGuess_<<" weight max: "<<maxw<<endl; 
    ...
    meanBest_  = minuit_->GetParameter(2); // The mean is allowed to float between (-1,2) to prevent
...
class MultiPeriodicAnalysisFCN : public ROOT::Minuit2::FCNBase {
 private:
 ...
 public:
  ...
  virtual void Point(AnalysisFn* fn) {
    ptr = dynamic_cast<MultiPeriodicAnalysisFn*>(fn); 
  }
};
