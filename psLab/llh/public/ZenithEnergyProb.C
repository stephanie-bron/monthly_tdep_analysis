...
void ZenithEnergyProb::SetTableGamma(TTree *srcTree,TCut cut,TString enString) //cut = 1
{
  if (recoZenRadName_ == "") {
  ...
  for (int zBand=1; zBand <= GetNZenithBands(); ++zBand) {
    ...
    TCut zenCut(zenString); //create a new cut for each zenith band, in order to select only events that have a zenith included in the band
    assert( VerifyExpression(srcTree, zenCut.GetTitle()) );

    eProbVect_[zBand].SetTableGamma(srcTree, zenCut*cut, enString); //recall the function in order to use the cut
  }
  ...
  (const Event& event, double gamma) const
{
  ...
  return eProbVect_[band].GetEnergyProbGamma(event, gamma); //for each zenith band, will get the energy prob for an event with hypothesis on gamma
}
